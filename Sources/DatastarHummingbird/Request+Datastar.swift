...
extension Request {
    /// Decode Datastar signals from the request, dispatching on HTTP method
    /// per the ADR:
    ///
    /// - `GET` and `DELETE` read from the `?datastar=<json>` query parameter.
    /// - `POST`, `PUT`, and `PATCH` read the JSON body (up to `context.maxUploadSize`).
