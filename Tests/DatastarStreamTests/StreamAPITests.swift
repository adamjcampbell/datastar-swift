...
@Suite("DatastarSSEStream closure init")
struct StreamAPITests {
    ...
    func happyPath() async throws {
        ...
        let output = try await collect(stream)
        #expect(output == """
    ...
    @Test("DatastarSSEStream(_:) accepts any async sequence of DatastarEvents")
    ...
    @Test("DatastarSSEStream(_:) accepts a sequence of any DatastarEventConvertible element")
    func fromAsyncSequenceOfPayloadStructs() async throws {
        ...
        let stream = DatastarSSEStream(patches)
        let output = try await collect(stream)
        #expect(output.contains("data: elements <p>a</p>"))
        #expect(output.contains("data: elements <p>b</p>"))
    }
    ...
    @Test("Dropping the stream cancels the producer closure and releases it")
    func consumerDropCancelsProducer() async throws {
        ...
        } // stream leaves scope here, TaskHolder is released, producer Task is cancelled
        ...
        #expect(exitedCount == 1,
                "producer closure must exit after the stream is released (did not exit within 500ms)")
    }

    @Test("Consumer task cancellation unblocks a parked receive")
    func consumerTaskCancellationUnblocksReceive() async throws {
        let stream = DatastarSSEStream { _ in
